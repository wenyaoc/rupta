...
use crate::util::renumber::{RegionCtxt, BorrowckInferCtxt};
...
impl<'tcx> UniversalRegions<'tcx> {
    ...
    pub fn closure_mapping(
        ...
        // for_each_late_bound_region_in_recursive_scope(tcx, tcx.parent(closure_def_id), |r| {
    ...
    pub(crate) fn annotate(&self, tcx: TyCtxt<'tcx>, err: &mut Diagnostic) {
        match self.defining_ty {
            DefiningTy::Closure(def_id, args) => {
                ...
                // for_each_late_bound_region_in_recursive_scope(tcx, def_id, |r| {
...
impl<'cx, 'tcx> UniversalRegionsBuilder<'cx, 'tcx> {
    fn build(self) -> UniversalRegions<'tcx> {
        ...
        assert_eq!(FIRST_GLOBAL_INDEX, self.infcx.infcx.num_region_vars());
        ...
        let first_extern_index = self.infcx.infcx.num_region_vars();
        ...
        let typeck_root_def_id = self.infcx.infcx.tcx.typeck_root_def_id(self.mir_def);

        // If this is a 'root' body (not a closure/coroutine/inline const), then
        // there are no extern regions, so the local regions start at the same
        // position as the (empty) sub-list of extern regions
        let first_local_index = if self.mir_def == typeck_root_def_id {
            first_extern_index
        } else {
            ...
            //     self.infcx.infcx.tcx.parent(self.mir_def),
            //     |r| {
            //         // // debug!(?r);
            ...
            self.infcx.infcx.num_region_vars()
        };
        ...
        // signature are local.
        // for_each_late_bound_region_in_item(self.infcx.infcx.tcx, self.mir_def, |r| { 
        //     // // debug!(?r);
        //     if !indices.indices.contains_key(&r) {
        //         let region_vid = {
        //             let name = r.get_name_or_anon();
        ...
        if let DefiningTy::FnDef(def_id, _) = defining_ty {
            if self.infcx.infcx.tcx.fn_sig(def_id).skip_binder().c_variadic() {
                let va_list_did = self.infcx.infcx.tcx.require_lang_item(
                    LangItem::VaList,
                    Some(self.infcx.infcx.tcx.def_span(self.mir_def)),
                );
                ...
                let region = ty::Region::new_var(self.infcx.infcx.tcx, reg_vid);
                let va_list_ty = self
                    .infcx
                    .infcx
                    .tcx
                    .type_of(va_list_did)
                    .instantiate(self.infcx.infcx.tcx, &[region.into()]);

                unnormalized_input_tys = self.infcx.infcx.tcx.mk_type_list_from_iter(
        ...
        let num_universals = self.infcx.infcx.num_region_vars();
    ...
    fn defining_ty(&self) -> DefiningTy<'tcx> {
        let tcx = self.infcx.infcx.tcx;
        let typeck_root_def_id = tcx.typeck_root_def_id(self.mir_def);
        ...
        match tcx.hir().body_owner_kind(self.mir_def) {
            ...
            BodyOwnerKind::Const { .. } | BodyOwnerKind::Static(..) => {
                ...
                if self.mir_def == typeck_root_def_id {
                    ...
                    DefiningTy::Const(self.mir_def, args)
                } else {
                    ...
                    //TODO: fIX THE BELOW
                    let ty = tcx
                        .typeck(self.mir_def.expect_local())
                        .node_type(tcx.local_def_id_to_hir_id(self.mir_def.expect_local())); //TODO: mir_def
                    ...
                    let args =
                        self.infcx.replace_free_regions_with_nll_infer_vars(FR, identity_args);
                    DefiningTy::InlineConst(self.mir_def, args)
                }
            }
        }
    }
        ...
        match defining_ty {
            ...
            DefiningTy::Coroutine(def_id, args) => {
                ...
                let inputs_and_output =
                    self.infcx.infcx.tcx.mk_type_list(&[coroutine_ty, resume_ty, output]);
...
impl<'cx, 'tcx> InferCtxtExt<'tcx> for BorrowckInferCtxt<'tcx> {
    ...
    where
        T: TypeFoldable<TyCtxt<'tcx>>,
    {
        let (value, _map) = self.infcx.tcx.instantiate_bound_regions(value, |br| {
            // debug!(?br);
            let liberated_region =
                ty::Region::new_late_param(self.infcx.tcx, all_outlive_scope, br.kind);
            let region_vid = {
...
fn for_each_late_bound_region_in_recursive_scope<'tcx>(
    tcx: TyCtxt<'tcx>,
    mut mir_def_id: DefId,
    mut f: impl FnMut(ty::Region<'tcx>),
) {
    let typeck_root_def_id = tcx.typeck_root_def_id(mir_def_id);
    ...
    loop {
        ...
        if mir_def_id == typeck_root_def_id {
            break;
        } else {
            mir_def_id = tcx.parent(mir_def_id);
        }
    }
}
...
fn for_each_late_bound_region_in_item<'tcx>(
    ...
    for bound_var in tcx.late_bound_vars(tcx.local_def_id_to_hir_id(mir_def_id.expect_local())) { //TODO: mir_def_id
        let ty::BoundVariableKind::Region(bound_region) = bound_var else {
            continue;
        };
        let liberated_region =
            ty::Region::new_late_param(tcx, mir_def_id, bound_region);
        f(liberated_region);
    }
