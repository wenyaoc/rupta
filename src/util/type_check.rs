...
pub fn type_check<'mir, 'tcx>(
    ...
    let implicit_region_bound = ty::Region::new_var(infcx.infcx.tcx, universal_regions.fr_fn_body);
    ...
    let user_type_annotations = &body.user_type_annotations;
    println!("user_type_annotations: {:?}", user_type_annotations);
    ...
    //             if !(infcx.infcx.tcx.features().unsized_locals || infcx.infcx.tcx.features().unsized_fn_params) {
...
fn check_stmt<'tcx>(
    ...
    match &stmt.kind {
        ...
        StatementKind::Intrinsic(box kind) => {
            // TODO: Implement this
            println!("Intrinsic");
        }
        // => match kind {
...
fn check_rvalue<'tcx>(
    ...
    match rvalue {
        ...
        Rvalue::Repeat(operand, len) => {
            println!("Repeat: operand={:?}, len={:?}", operand, len);
        ...
        Rvalue::Cast(cast_kind, op, ty) => {
            println!("Cast: cast_kind={:?}, op={:?}, ty={:?}", cast_kind, op, ty);
        ...
        | Rvalue::CheckedBinaryOp(_, box (left, right)) => {
            println!("BinaryOp/CheckedBinaryOp");
            // self.check_operand(left, location);
            // self.check_operand(right, location);
        }
        ...
        | Rvalue::NullaryOp(NullOp::OffsetOf(..), _) => {
            println!("AddressOf/ThreadLocalRef/Len/Discriminant/NullaryOp");
        }
    }
}
...
//         // When `unsized_fn_params` or `unsized_locals` is enabled, only function calls
//         // and nullary ops are checked in `check_call_dest`.
//         if !(features.unsized_locals || features.unsized_fn_params) {
//             let span = local_decl.source_info.span;
//             let ty = local_decl.ty;
//             self.ensure_place_sized(ty, span);
//         }
...
impl PoloniusLocationTable {
    pub(crate) fn new(body: &Body<'_>) -> Self {
        ...
        // println!("PoloniusLocationTable(statements_before_block={:#?})", statements_before_block);
